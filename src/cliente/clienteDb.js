...
async function atualizarCliente(event,id,cpf,senha,email) {
    const resultado = await db.query('update sistema.cliente set cpf = $2, senha = $3, email = $4 where id = $1',[id,cpf,senha,email])
     console.log(event)
    return resultado.rows;
}
